  ...
  export default {
    ...
    computed: {
      ...
      investments() {
        return this.investmentsHistory.map((i) => ({
          ...
          investmentOpportunity: this.$store.getters['investmentOpportunity/one'](i.investmentOpportunity._id)
        }));
      }
    },
    ...
    methods: {
      ...
      async getInvestorsByInvestmentOpportunityIds(investmentOpportunityIds) {
        try {
          const action = (id) => this.$store.dispatch('investmentOpportunity/getInvestmentOpportunityInvestments', id);
      ...
      async getInvestmentsHistoryByUserId(id) {
        try {
          return this.$store.dispatch('investmentOpportunity/getInvestmentsHistory', id);
        } catch (error) {
          console.error(error);
