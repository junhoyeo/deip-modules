...
<script>
  import { CreateCrowdfundingForm } from '@deip/investment-opportunity-module';
  ...
  export default {
    ...
    async created() {
      ...
      const activeInvestmentOpportunities = this.$store.getters['investmentOpportunity/list']({
    ...
    methods: {
      ...
      async getInvestmentOpportunities() {
        try {
          await this.$store.dispatch('investmentOpportunity/getCurrentInvestmentOpportunityByProject', this.project._id);
        } catch (error) {
          console.error(error);
        }
      },
